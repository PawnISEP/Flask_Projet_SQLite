document.addEventListener("DOMContentLoaded", () => {
    ...
    const searchBar = document.getElementById("searchBar"); // Recherche par titre de livre
    const searchUserInput = document.getElementById("searchUser"); // Recherche par nom d'utilisateur
    const rows = document.querySelectorAll(".table-catalogue tbody tr");
    ...
    if (requestSuccess) {
        showRequestNotification("Votre démarche a été effectuée avec succès !");
    ...
    // Recherche dynamique dans le tableau par titre de livre
    if (searchBar) {
    ...
    // Recherche dynamique dans le tableau par nom d'utilisateur
    if (searchUserInput) {
        searchUserInput.addEventListener("input", () => {
            ...
            rows.forEach(row => {
                const userElement = row.querySelector(".nom-utilisateur");
                if (userElement) {
                    const userText = userElement.textContent.toLowerCase();
                    if (userText.includes(userQuery)) {
                        row.style.display = ""; // Afficher
                        hasUserResults = true;
                    } else {
                        row.style.display = "none"; // Masquer
            ...
            noResultsMessage.style.display = hasUserResults ? "none" : "block";
        });
    }
});
